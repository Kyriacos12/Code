...
def init(engine, directory):
    ...
    filename = '{0}\\Networks\\Paparia\\Loadshapes\\'.format(str(directory))
    engine.text.command = 'set datapath=' + filename
    engine.text.command = 'redirect Loadshapes.txt'

    filename = '{0}\\Networks\\Paparia\\63057169\\'.format(str(directory))
    engine.text.command = "set datapath=" + filename
    engine.text.command = 'redirect Master.txt'

    filename = '{0}\\Networks\\Paparia\\63057170\\'.format(str(directory))
    engine.text.command = 'set datapath=' + filename
    engine.text.command = 'redirect Master.txt'

    filename = '{0}\\Networks\\Paparia\\63057171\\'.format(str(directory))
    engine.text.command = 'set datapath=' + filename
    engine.text.command = 'redirect Master.txt'

    filename = '{0}\\Networks\\Paparia\\63057172\\'.format(str(directory))
    engine.text.command = 'set datapath=' + filename
    engine.text.command = 'redirect Master.txt'

    filename = '{0}\\Networks\\Paparia\\63057173\\'.format(str(directory))
    engine.text.command = "set datapath=" + filename
    engine.text.command = 'redirect Master.txt'

    filename = '{0}\\Networks\\Paparia\\63057174\\'.format(str(directory))
    engine.text.command = 'set datapath=' + filename
    engine.text.command = 'redirect Master.txt'
...
def export_monitors(engine, iteration, directory):
    ...
    if not os.path.exists(directory):
        ...
        os.makedirs(directory)
    elif not os.path.exists((directory + "\\Paparia")):
        directory += "\\Paparia"
        os.makedirs(directory)
        directory += "\\%s" % iteration
        os.makedirs(directory)
    elif not os.path.exists((directory + "\\Paparia\\%s" % iteration)):
        directory += "\\Paparia\\%s" % iteration
        os.makedirs(directory)
    else:
        directory += "\\Paparia\\%s" % iteration
